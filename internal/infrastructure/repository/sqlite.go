...
type RequestLogRow struct {
	...
	UpstreamRespHeaders string `gorm:"type:text"`
	UpstreamRespBody    string `gorm:"type:text"`
...
func (r *SQLiteRequestLogRepository) EndpointErrorStats(ctx context.Context, filter domain.RequestLogFilter, limit int) ([]domain.EndpointErrorStat, error) {
	q := r.db.WithContext(ctx).Model(&RequestLogRow{})
	if strings.TrimSpace(filter.Path) != "" {
		q = q.Where("downstream_path = ?", strings.TrimSpace(filter.Path))
	}
	...
	items := make([]domain.RequestLog, 0, len(rows))
	for _, row := range rows {
...
func toRow(log domain.RequestLog) RequestLogRow {
	return RequestLogRow{
		...
		UpstreamRespStatus:  log.UpstreamRespStatus,
		UpstreamRespHeaders: log.UpstreamRespHeaders,
		UpstreamRespBody:    log.UpstreamRespBody,
...
func fromRow(row RequestLogRow) domain.RequestLog {
	return domain.RequestLog{
		...
		UpstreamRespStatus:  row.UpstreamRespStatus,
		UpstreamRespHeaders: row.UpstreamRespHeaders,
		UpstreamRespBody:    row.UpstreamRespBody,
